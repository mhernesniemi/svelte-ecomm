...
import { calculateDiscount, calculateProductDiscount, canCombinePromotions } from "./promotion-utils";
...
describe("calculateDiscount", () => {
	...
	it("calculates fixed amount discount", () => {
		expect(
			calculateDiscount({ discountType: "fixed_amount", discountValue: 500 }, 10000)
		).toBe(500);
	});

	it("caps fixed amount at order amount", () => {
		expect(
			calculateDiscount({ discountType: "fixed_amount", discountValue: 5000 }, 3000)
		).toBe(3000);
	});
...
describe("calculateProductDiscount", () => {
	...
	it("handles zero qualifying total", () => {
		expect(
			calculateProductDiscount({ discountType: "percentage", discountValue: 50 }, 0)
		).toBe(0);
	});
});
