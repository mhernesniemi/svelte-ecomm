<script lang="ts">
  ...
  interface Props {
    open: boolean;
    onClose: () => void;
    onSelect: (files: { url: string; name: string; fileId: string; width: number; height: number; size: number }[]) => void;
  }
  ...
  async function handleFileUpload(event: Event) {
    ...
    try {
      ...
      const uploadedFiles: { url: string; name: string; fileId: string; width: number; height: number; size: number }[] = [];
...
<Dialog.Root bind:open onOpenChange={handleOpenChange}>
  <Dialog.Content class="max-w-2xl">
    <Dialog.Header>
      ...
      <Dialog.Description>Upload new images or select from existing ones</Dialog.Description>
    </Dialog.Header>
