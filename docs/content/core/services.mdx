...
// Get by ID or slug (pass language for translation resolution)
const product = await productService.getById(123, locals.language);
const product = await productService.getBySlug("blue-shirt", locals.language);
...
// Search
const results = await productService.list({ search: "shirt", language: locals.language });
