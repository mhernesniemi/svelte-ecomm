...
├── lib/
│   ├── config/
│   │   └── languages.ts     # Language config (DEFAULT_LANGUAGE, LANGUAGES, translationsToMap)
│   ├── server/
...
│   │   │   ├── customers.ts
│   │   │   ├── collections.ts
│   │   │   ├── translations.ts  # Multi-language TranslationService
...
│   │   └── admin/
│   │       └── TranslationEditor.svelte  # Reusable translation editing UI
...
export const load = async ({ params }) => {
	...
	return { product }; // product.name is already resolved
};
...
└────────────────────────┬────────────────────────────────┘
                         ▼
┌─────────────────────────────────────────────────────────┐
│  services/*.ts (business logic + i18n resolution)       │
│  - resolves translations via i18n.ts                    │
│  - returns Resolved* types with flat fields             │
└────────────────────────┬────────────────────────────────┘
                         ▼
┌─────────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────────┘
                         ▲
                         │ resolved data (product.name)
                         ▼
┌─────────────────────────────────────────────────────────┐
│  +page.svelte (view)                                    │
│  - accesses flat fields: product.name, collection.slug  │
