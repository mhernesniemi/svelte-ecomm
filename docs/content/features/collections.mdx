...
```typescript
// Get collection by slug (pass language for translation resolution)
const collection = await collectionService.getBySlug("summer-sale", locals.language);
...
const { items, pagination } = await collectionService.getProductsForCollection(
  collection.id,
  { limit: 20, offset: 0, language: locals.language }
);
```
